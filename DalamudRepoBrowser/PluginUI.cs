...
public static class PluginUI
{
    ...
    private static uint filteredCount;
    ...
    public static bool AddHeaderIcon(string id, string icon)
    {
        ...
        var buttonPos =
            new Vector2(ImGui.GetWindowWidth() - buttonSize.X - 17 * scale - ImGui.GetStyle().FramePadding.X * 2, 2);
        ...
        drawList.AddText(UiBuilder.IconFont, ImGui.GetFontSize(),
            itemMin + halfSize - ImGui.CalcTextSize(icon) / 2 + Vector2.One, 0xFFFFFFFF, icon);
        ImGui.PopFont();
    ...
    public static void Draw()
    {
        ...
        if (DalamudRepoBrowser.sortList > 0 && --DalamudRepoBrowser.sortList <= 0)
        {
            ...
            {
                0 => DalamudRepoBrowser.repoList, // Stars (disabled)
                1 => DalamudRepoBrowser.repoList.OrderBy(r => r.owner).ToList(),
                2 => DalamudRepoBrowser.repoList.OrderBy(r => r.url).ToList(),
                3 => DalamudRepoBrowser.repoList.OrderByDescending(r => r.plugins.Count).ToList(),
                4 => DalamudRepoBrowser.repoList.OrderByDescending(r => r.lastUpdated).ToList(),
                _ => DalamudRepoBrowser.repoList
            };

            if (DalamudRepoBrowser.DalamudRepoSettings != null)
                enabledRepos = DalamudRepoBrowser.repoList.Where(r =>
                        DalamudRepoBrowser.GetRepoEnabled(r.url) || DalamudRepoBrowser.GetRepoEnabled(r.rawURL))
                    .ToHashSet();

            DalamudRepoBrowser.repoList.ForEach(r => DalamudRepoBrowser.Config.SeenRepos.Add(r.url));
            DalamudRepoBrowser.repoList = DalamudRepoBrowser.repoList
                .OrderBy(r => DalamudRepoBrowser.prevSeenRepos.Contains(r.url)).ToList();
        ...
        if (ImGui.Button(FontAwesomeIcon.Globe.ToIconString()))
            ...
            {
                FileName = @"https://beslightly.github.io/Aetherfeed/",
        ...
        if (ImGui.InputTextWithHint("##Search", $"Search {filteredCount} / {DalamudRepoBrowser.repoList.Count} Repos",
                ref searchText, 64))
            lock (DalamudRepoBrowser.repoList)
            {
                search = DalamudRepoBrowser.repoList.Where(r
                    => r.url.Contains(searchText, StringComparison.CurrentCultureIgnoreCase)
                       ...
                       || r.plugins.Any(plugin =>
                           plugin.name.Contains(searchText, StringComparison.CurrentCultureIgnoreCase)
                           ...
                           || plugin.categoryTags.Any(s =>
                               s.Equals(searchText, StringComparison.CurrentCultureIgnoreCase)))).ToHashSet();
            }
        ...
        if (openSettings)
        {
            ...
            // --- FIX IS HERE ---
            // We use a local variable to pass to the ref parameter, then update the config.
            var showOutdated = DalamudRepoBrowser.Config.ShowOutdatedPlugins;
            if (ImGui.Checkbox("Show Outdated Plugins", ref showOutdated))
            {
                DalamudRepoBrowser.Config.ShowOutdatedPlugins = showOutdated;
                save = true;
            }
            ...
            ImGui.SameLine();
            save |= ImGui.RadioButton("Last Updated", ref DalamudRepoBrowser.Config.RepoSort, 4);

            ImGui.NextColumn();
            ImGui.TextUnformatted("");

            // --- FIX IS HERE ---
            // Applying the same local variable pattern to the other checkbox.
            var hideEnabled = DalamudRepoBrowser.Config.HideEnabledRepos;
            if (ImGui.Checkbox("Hide Enabled Repos", ref hideEnabled))
            {
                DalamudRepoBrowser.Config.HideEnabledRepos = hideEnabled;
                save = true;
            }

            ImGui.SetNextItemWidth(ImGui.GetWindowWidth() / 5);
            save |= ImGui.SliderInt(
                DalamudRepoBrowser.Config.MaxPlugins >= 50 ? "∞ Plugins###MaxPlugins" : "Maximum Plugins###MaxPlugins",
                ref DalamudRepoBrowser.Config.MaxPlugins, 20, 50);
        ...
        lock (DalamudRepoBrowser.repoList)
        {
            ...
            {
                // --- REWORKED FILTER LOGIC ---
                var hasAnyValidPlugins = repoInfo.plugins.Any(p => p.apiLevel == DalamudRepoBrowser.currentAPILevel);

                if (!DalamudRepoBrowser.Config.ShowOutdatedPlugins && !hasAnyValidPlugins) continue;

                if (DalamudRepoBrowser.Config.MaxPlugins < 50 &&
                    DalamudRepoBrowser.Config.MaxPlugins < repoInfo.plugins.Count) continue;
                if (doSearch && !search.Contains(repoInfo)) continue;

                var enabled = DalamudRepoBrowser.GetRepoEnabled(repoInfo.url) ||
                              DalamudRepoBrowser.GetRepoEnabled(repoInfo.rawURL);
                ...
                if (!seen)
                    ImGui.GetWindowDrawList().AddRectFilledMultiColor(ImGui.GetItemRectMin(),
                        new Vector2(ImGui.GetWindowPos().X + ImGui.GetWindowSize().X,
                ...
                if (ImGui.Checkbox($"{repoInfo.url}##Enabled", ref enabled))
                    DalamudRepoBrowser.ToggleRepo(DalamudRepoBrowser.HasRepo(repoInfo.rawURL)
                        ? repoInfo.rawURL
                        : repoInfo.url);
                ...
                {
                    ImGui.SameLine();
                    var lastUpdatedStr = DateTimeOffset.FromUnixTimeSeconds(repoInfo.lastUpdated).LocalDateTime
                        .ToString("yyyy-MM-dd");
                ...
                foreach (var plugin in repoInfo.plugins)
                {
                    ...
                    if (!DalamudRepoBrowser.Config.ShowOutdatedPlugins && !valid) continue;
                    ...
                    textMax.X += padding;
                    var drawList = ImGui.GetWindowDrawList();
                    drawList.AddRectFilled(textMin, textMax, (uint)(valid ? 0x20FFFFFF : 0x200000FF),
                    ...
                    if (!valid)
                    {
                        ...
                        drawList.AddLine(new Vector2(textMin.X, textMax.Y), new Vector2(textMax.X, textMin.Y), color,
                            thickness);
                    }
                    ...
                    if (ImGui.IsItemHovered())
                    {
                        ...
                        if (hasRepo && ImGui.IsMouseReleased(ImGuiMouseButton.Left) &&
                            plugin.repoURL.StartsWith(@"http"))
                            try
                            {
                                Process.Start(new ProcessStartInfo
                                {
